...
function Rventas() {
  ...
  const hacerVenta = () => {
    ...
    console.log(token);
    axios
      .post("http://165.22.189.59:8001/api/venta", venta, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then((response) => {
        ...
        // Recorre el array carrito y envía los datos a la ruta "http://165.22.189.59:8001/api/asigna"
        carrito.forEach((producto) => {
          ...
          console.log(token);
          axios
            .post("http://165.22.189.59:8001/api/asigna", asignacion, {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            })
